...
def member_savings(request):
    members_with_savings = Member.objects.annotate(total_savings=Sum('deposit__amount'))
    members_with_fines = Fine.objects.annotate(total_savings=Sum('finepayment__amount'))
    members_with_deposits = Deposit.objects.filter(status='approved').annotate(total_savings=Sum('amount'))
    context ={
            'members_with_savings': members_with_savings,
            'members_with_fines':members_with_fines,
            'members_with_deposits':members_with_deposits,

    }
    return render(request, 'members/member_savings.html', context)

def report(request):
    all_members = Member.objects.all
    all_deposits = Deposit.objects.all().order_by("-date_created")
    ...
    # cursor.execute('select sum(amount) from kyaclub_app_fine union select sum(amount) from kyaclub_app_deposit')
    # f = cursor.fetchone()
...
def home(request): 
    ...
    total_savings = Deposit.objects.all().aggregate(Sum('amount'))
    ...
    member_balance = Member.objects.filter(deposit__status="Approved").annotate(total_savings=Sum('deposit__amount'))
